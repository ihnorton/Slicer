...
void* vtkNRRDWriter::MakeNRRD()
  ...
  // use double-conversion library (via ITK) for more accurate
  ...
  if (this->DiffusionWeightedData)
    ...
    if (kind[0] == nrrdKindList && numGrad == size[0] && numBValues == size[0])
      ...
      bval_value << setprecision(17) << maxbVal;
      ...
      for (unsigned int ig =0; ig < numGrad; ig++)
        ...
        // the scale factor must be applied to normalized gradients
        grad.normalize();
        ...
        value_stream << std::setprecision(17) << std::scientific <<
