...
{
  if (vtkPolyData* polydata = vtkPolyData::SafeDownCast(this->GetMesh()))
    {
    return polydata;
    }
  else
...
{
  return vtkUnstructuredGrid::SafeDownCast(this->GetMesh());
}
...
::SetPolyDataConnection(vtkAlgorithmOutput *newPolyDataConnection)
{
  ...
  this->GeometryFilter->SetInputConnection(0);
}
...
::SetUnstructuredGridConnection(vtkAlgorithmOutput *newUnstructuredGridConnection)
{
  ...
  this->GeometryFilter->SetInputConnection(this->MeshConnection);
}
...
vtkAlgorithmOutput* vtkMRMLModelNode::GetPolyDataConnection()
{
  return (this->MeshType == vtkMRMLModelNode::PolyDataMeshType) ?
    this->GetMeshConnection() : this->GeometryFilter->GetOutputPort();
}
...
void vtkMRMLModelNode::ApplyTransform(vtkAbstractTransform* transform)
{
  ...
  if (this->MeshType == vtkMRMLModelNode::UnstructuredGridMeshType)
    {
    this->GeometryFilter->SetInputConnection(this->MeshConnection);
    }
}
